...
function getMinutes(busID, callback){
    var returnString = 'error';
		let apiAddress = 'https://api.wmata.com/NextBusService.svc/json/jPredictions?api_key=6b700f7ea9db408e9745c207da7ca827&StopID='+busID;
    const req = https.request(apiAddress, (res) => {
        let body = '';
...
exports.handler = (event, context, callback) => {
    console.log('Received event:', event.clickType);
		...
		connection.query('SELECT * FROM kidtracker where id="123"',function(err,results) {
		  if (err) throw err;

		  console.log(results[0].id);
		  console.log(results[0].parentName);
		  var parentName = results[0].parentName;
		  var childName = results[0].childName;
			var EMAIL = results[0].email;
			var busID = results[0].pickUp;
		});

    var minutes = getMinutes(busID, callback);
    console.log(minutes);
    // create/get topic
